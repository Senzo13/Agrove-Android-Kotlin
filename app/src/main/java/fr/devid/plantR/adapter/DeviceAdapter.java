package fr.devid.plantR.adapter;
...
public class DeviceAdapter extends BaseAdapter {
    ...
    public void addDevice(BleDevice bleDevice) {
        ...
        Timber.d("La key " + bleDevice.getKey());

        String countAddDevice = "";
        Integer nbWse = 0;
        for(Integer i = 0; i <= 2; i++) {
            countAddDevice += String.valueOf(bleDevice.getKey().charAt(i));
            Timber.d("Couscous : " + countAddDevice);
            if (countAddDevice.equals("WSE") || countAddDevice.equals("WSH") || countAddDevice.equals("Flo")) {
                nbWse++;
                Timber.d("J'ajoute : " + nbWse + " device wse ou device wsh ou device flower care");

                bleDeviceList.add(bleDevice);
            }
        }

       //if(bleDevice.getKey().equals("WSE-MT-B01_0006E3:B7:87:62:DB:7C")) {
       //}
    }
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (convertView != null) {
            ...
            holder.ble_list = (RelativeLayout) convertView.findViewById(R.id.ble_list);
        ...
        if (bleDevice != null) {
            ...
            Integer nbWse = 0;
            for(Integer i = 0; i <= 2; i++) {
                count += String.valueOf(bleDevice.getKey().charAt(i));
                Timber.d("Nom :" + count);
                if(count.equals("WSE")) {
                    nbWse++;
                    Timber.d("Nous avons trouvé " + nbWse + " appareils agrove !");
                    String name = "Agrove " + bleDevice.getKey().charAt(bleDevice.getName().length() - 4) + bleDevice.getKey().charAt(bleDevice.getName().length() - 3) +bleDevice.getKey().charAt(bleDevice.getName().length() - 2) +bleDevice.getKey().charAt(bleDevice.getName().length() - 1)  ;
